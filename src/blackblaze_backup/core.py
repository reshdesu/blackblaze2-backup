...
class BackupConfig:
    ...
    def get_backup_plan(self) -> Dict[str, str]:
        ...
        if self.single_bucket_mode:
            return {folder: self.single_bucket_name for folder in self.folders_to_backup.keys()}
        else:
...
class BackupService:
    ...
    def execute_backup(self, progress_callback=None, status_callback=None, error_callback=None) -> bool:
