...
class TestCredentialsTab:
    ...
    def test_test_connection_success(self, mock_set_credentials, mock_message_box, app, qtbot):
        ...
        test_button = next((btn for btn in buttons if btn.text() == "Test Connection"), None)
    ...
    @patch.object(BackupService, 'set_credentials')
    def test_test_connection_failure(self, mock_set_credentials, mock_message_box, app, qtbot):
...
class TestFoldersTab:
    ...
    def test_add_folder_dialog(self, app, qtbot, temp_folder):
        ...
        with patch('blackblaze_backup.gui.QFileDialog.getExistingDirectory') as mock_dialog:
            mock_dialog.return_value = str(temp_folder)
            
            # Find and click add folder button
            buttons = app.findChildren(QPushButton)
            add_button = next((btn for btn in buttons if btn.text() == "Add Folder"), None)
...
class TestBackupTab:
    ...
    def test_start_backup_no_credentials(self, mock_message_box, app, qtbot):
        ...
        buttons = app.findChildren(QPushButton)
        start_button = next((btn for btn in buttons if btn.text() == "Start Backup"), None)
...
class TestIntegration:
    ...
    def test_complete_workflow(self, mock_validate, mock_add_folder, mock_set_credentials, 
                              mock_message_box, app, qtbot, temp_folder):
