...
class TestSuite:
    ...
    def run_gui_tests(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["uv", "run", "pytest", "tests/test_gui.py", "-v", "--maxfail=3"],
                cwd=self.project_root,
                capture_output=True,
                text=True,
            ...
            if env_file.exists():
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        print("🧹 Test files were automatically cleaned up after verification")
                    else:
                        print("⚠️  Real S3 connectivity test failed (using placeholder credentials)")
                        print("🔄 Falling back to simulation...")
                        print("🔄 Simulating backup process...")
                        time.sleep(1)  # Simulate processing time
                        print("✅ Backup simulation completed successfully")
                except subprocess.TimeoutExpired:
                    print("⏰ S3 connectivity test timed out, falling back to simulation")
                    print("🔄 Simulating backup process...")
                    time.sleep(1)  # Simulate processing time
                    print("✅ Backup simulation completed successfully")
                except Exception as e:
                ...
                print("💡 To test real connectivity, create a .env file with your BackBlaze B2 credentials")
                print("🔄 Simulating backup process...")
                time.sleep(1)  # Simulate processing time
                print("✅ Backup simulation completed successfully")
    ...
    def test_credentials(self) -> bool:
        ...
        try:
            ...
            is_valid, message = credential_manager.validate_credentials(test_credentials)
